...
Looking at the data, can we find ways to better distribute the money?


Where do we have funcing gaps?
Are the funds reaching people that need them the most?

Transparent about assumptuoons and intentions.

Help decision makers ask better questions, not just give them answers.

Datasets represent actual people, so we need to be careful about how we use them. We need to make sure we're not reinforcing existing biases or inequalities.

Dashbords, visualizations, and reports that are easy to understand and use for decision making.

Be cautios about predictions and overconfidence in the models. Focus on decision support rather than making predictions about the future.

How would this be used by decision makers? What questions would they ask? How would they use the insights to make better decisions?

1. Global Humanitarian Porgramme Cycle - how we target people in need in distribution of help (water, shelter, food, medical care, etc.)

2. Humanitarian Response Plan - What we propose to deliver (this one may need look at methodology, which can change depend on country)

3. Global Subnational Populatrion Statistics - common operational dataset, breakdown about dataset on each one of target areas, disatsters often doesnt have good data, it is often based on models (when was the last census? in sudan in 199something))

4. Global requirements and funcing dtaa (main) - dollar amount on how much many this services will cost, 2bn dollars or sth loke that, split between diffrent areas (300m for water, 500m for health)

5. Country based pooled funds data hub - the funding system has been thensformed over the lat few years, now we have big donors to the entire pool rather than individual projects FOLLOW THE MONEY

Is the situation in the country geeting better or worse? 

Most humanitarain help goes at subnational level (like only some parts of Sudan)

Shape of solutions:
- lot of diffrent things
- we sould not make a dashbord? dashbor theater is not good, level of detail is usually far to general or far to complex that end users have difficulity getting results. Avoid good looking but not useful dashboards.
- What is the question we are trying to answer with this decision tools?
- Something that produces a 500 word paragraps or spreadsheets that has something usefull is far more productive.

Q&A:
- Severity score seems to be retrospective, what data use close to an outbreak? We need to be able to make decisions in real time, not just after the fact.

Most recent hum .. overv dataset has the newest. 

needs assesment data - immediately ofter news

they are unique to contries (so needs assesment for each specific country is better than a general model)

the data is usually not structured, it is often a report reliefweb!!! assesment data, assesment reports after disatsers (people just reporting what happens, it is excited as the way how we find data is key)

- user experience vs paragraph: best of all complex analysis packed with visuals

how to make usefull balance between visal and engaging and substantive information? combinig is best of both words but data transformations and models own or 3 part we need to be clear how they are used in final project

we understand that we are in an era where tools and ai fail quietly, back in the day we saw when a number didnt made sense, now it is hard to unpick the logic of the model or pipeline itself, be cautios of how our pipelone work!

- funding allocations example: hurracane hits -> funding allocated
natural disaster hapopens, earthwuaqe, cyclon etc -> someone has to declare an emergancy (avalance in swiss alps wont get funding), the country has to request a need for fundingn assistance, then depending on the meachnism they requested funding from, they will either CERF (ONE TIME big allocation - major flood 20m to get head above water, the the head of OCHA will say yes and give finds rom the big poll and the humanitarian coordinator in that country will split he money at country leve),

poll funds for complex emergencies at country level (sudan, myenmar, chad, ) - they have ongoing operations for several years, they have a ig pool and people request money like a grand (i want to build a shelter, so i require money to buld it)


- follow up: money from second pool (are we asuuming that we are people who are in charge of the pool or are we people who are requesting money from the pool?)

UN get into a supply side problem, there are good organizations specified at shelters or vaccines, they will request funding and un gives them funds, but it does not be the place where the biggest demand is, the gap is THE MOST INTERESITNG, gap between who has greatest needs and who is getting the most funds

- detail into target audience for our audience and what techniqual level

target humanitarian coordinator, senior official in each country, tehy are able to aggregate a lot informations and make decisions quickly, we want to give them the most valuable data possible, but it is only one part of the puzzle, they also make decision based on their own experiencr, working in this field for 10-20 years they have knowledge, how do we help them? 

10 new project on desk, we have funding for 5, we have to decide which 5 to fund

technical level: there is an incedibre use and abuse of math, using math in a ways that (the book) 

one point is to be clear how we derive our figures and statistics (explain in clear language how we get to the numbers, what data we use, what assumptions we make, what models we use, etc.)

documenting the metodology is as important as an output

our audience are not mathematicians

they work in places where connectivity can go out any time, so we should accomodate for unusual scenarios in thet regard?


- reports how the granting went, was there over or underevaluations, feedback-loop-evaluations, we can fid wen on reliefweb and unocha!!

DATABRICKS

we dont have to use it but they recommend using it, standard notebooks mixing data and loke that, we should not reack the limit, if so find somebody to expamnd limit
